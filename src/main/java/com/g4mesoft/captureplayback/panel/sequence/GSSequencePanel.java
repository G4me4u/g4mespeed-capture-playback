...
//	public static final int DARK_COLUMN_COLOR = 0xDA0A0A0A;
	...
	private static final Text CREATE_ENTRY_TEXT = Text.translatable("panel.sequencecontent.createentry");
	private static final Text ENTRY_TYPE_TEXT   = Text.translatable("panel.sequencecontent.entrytype");
	private static final Text DELETE_ENTRY_TEXT = Text.translatable("panel.sequencecontent.deleteentry");
	...
	public void populateRightClickMenu(GSDropdown dropdown, int x, int y) {
		...
		if (entry != null) {
			for (GSEChannelEntryType type : GSEChannelEntryType.TYPES) {
				...
				Text text = Text.translatable(type.getName());
