...
//	public static final int DARK_COLUMN_COLOR = 0xDA0A0A0A;
	...
	private static final Component CREATE_ENTRY_TEXT = GSTextUtil.translatable("panel.sequencecontent.createentry");
	private static final Component ENTRY_TYPE_TEXT   = GSTextUtil.translatable("panel.sequencecontent.entrytype");
	private static final Component DELETE_ENTRY_TEXT = GSTextUtil.translatable("panel.sequencecontent.deleteentry");
	...
	public void render(GSIRenderer2D renderer) {
		...
		renderColumns(renderer, bounds);
		renderChannels(renderer, bounds);
		
		if (editable) {
			GSChannel draggedChannel = sequence.getChannel(draggedChannelUUID);
			if (draggedChannel != null) {
				renderDraggedChannel(renderer, draggedChannel, bounds);
			} else {
	...
	protected void renderColumns(GSIRenderer2D renderer, GSRectangle bounds) {
		renderColumns(renderer, bounds.y, bounds.height, bounds);
	}
	...
	protected void renderColumns(GSIRenderer2D renderer, int y, int h, GSRectangle bounds) {
		if (y + h >= bounds.y && y - bounds.y < bounds.height) {
			...
			while (cx < bounds.x + bounds.width) {
				...
				renderColumn(renderer, columnIndex, cx, y, cw, h);
				cx += cw;
				columnIndex++;
			}
		}
	...
	protected void renderColumn(GSIRenderer2D renderer, int columnIndex, int cx, int y, int cw, int height) {
		...
		if (expandedColumnModel.isColumnExpanded(columnIndex)) {
			// Always start line dashes relative to y = 0 (i.e. offset the y-coordinate).
			int ly = y + DOTTED_LINE_SPACING / 2 - (y % (DOTTED_LINE_LENGTH + DOTTED_LINE_SPACING));
	...
	protected void renderChannels(GSIRenderer2D renderer, GSRectangle bounds) {
		for (GSChannel channel : sequence.getChannels()) {
			...
			if (cy + modelView.getChannelHeight() >= bounds.y && cy - bounds.y < bounds.height) {
				if (channel.getChannelUUID().equals(draggedChannelUUID)) {
					int h = modelView.getChannelHeight() + modelView.getChannelSpacing();
					renderer.fillRect(bounds.x, cy, bounds.width, h, COLUMN_COLOR);
				} else {
					renderChannel(renderer, channel, cy, bounds);
				}
			}
		}
	}
	...
	protected void renderDraggedChannel(GSIRenderer2D renderer, GSChannel draggedChannel, GSRectangle bounds) {
		renderColumns(renderer, draggedChannelY, modelView.getChannelHeight(), bounds);
		renderChannel(renderer, draggedChannel, draggedChannelY, bounds);
	}
	...
	protected void renderChannel(GSIRenderer2D renderer, GSChannel channel, int cy, GSRectangle bounds) {
		...
		for (GSChannelEntry entry : channel.getEntries())
			renderChannelEntry(renderer, entry, channel.getInfo().getColor(), entryOffsetY, bounds);
		renderMultiCells(renderer, channel.getChannelUUID(), cy, bounds);
	...
	protected void renderMultiCells(GSIRenderer2D renderer, UUID channelUUID, int cy, GSRectangle bounds) {
		...
		while (itr.hasNext()) {
			...
			if (!expandedColumnModel.isColumnExpanded(multiCellInfo.getColumnIndex()))
				renderMultiCell(renderer, cy, multiCellInfo, bounds);
		}
	}
	...
	protected void renderMultiCell(GSIRenderer2D renderer, int cy, GSMultiCellInfo multiCellInfo, GSRectangle bounds) {
		...
		if (cx + cw >= bounds.x && cx - bounds.x < bounds.width) {
			...
			renderer.drawCenteredText(infoText, cx + cw / 2, ty, TEXT_COLOR);
		}
	}
	...
	public void populateRightClickMenu(GSDropdown dropdown, int x, int y) {
		...
		if (entry != null) {
			for (GSEChannelEntryType type : GSEChannelEntryType.TYPES) {
				...
				Component text = GSTextUtil.translatable(type.getName());
	...
	public void mousePressed(GSMouseEvent event) {
		if (event.getButton() == GSMouseEvent.BUTTON_LEFT) {
			...
			if (!event.isConsumed() && editable && hoveredEntry != null) {
	...
	public void mouseDragged(GSMouseEvent event) {
		if (!event.isConsumed() && editable && event.getButton() == GSMouseEvent.BUTTON_LEFT) {
