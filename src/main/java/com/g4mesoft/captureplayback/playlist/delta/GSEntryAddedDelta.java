...
public class GSEntryAddedDelta extends GSEntryDelta {
	...
	public GSEntryAddedDelta(GSPlaylistEntry entry, UUID prevUUID) {
		this(entry.getEntryUUID(), prevUUID, entry.getType(), entry.getData());
	}
	
	public GSEntryAddedDelta(UUID entryUUID, UUID prevUUID, GSEPlaylistEntryType type, GSIPlaylistData data) {
		super(entryUUID);
